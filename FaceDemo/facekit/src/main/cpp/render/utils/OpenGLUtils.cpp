...
namespace face {
    ...
    GLuint OpenGLUtils::loadProgram(const char *vShaderStr, const char *fShaderStr) {
        ...
        checkGLErrors("glAttachVertexShader");
        glAttachShader(programID, fragmentShader);
        checkGLErrors("glAttachFragmentShader");
        GLint linked;
    ...
    bool OpenGLUtils::createTextures(GLuint *textures, int num, int width, int height, bool isYuv) {
        ...
        for (uint32_t i = 0; i < num; i++) {
            ...
            glTexImage2D(GL_TEXTURE_2D, 0, format, widthTemp, heightTemp, 0, format,
    ...
    bool OpenGLUtils::attachmentTexture(GLuint textureId) {
        ...
        if (status != GL_FRAMEBUFFER_COMPLETE) {
            LOGE("OpenGLUtils [FrameBuffer] init error. status = %d , textureId=%d", status,
                 textureId);
        }
        return true;
    }
    ...
    void OpenGLUtils::readPixel(GLint x,
                                GLint y,
                                GLsizei width,
                                GLsizei height,
                                GLenum format,
    ...
    void OpenGLUtils::initDefaultVBO(GLuint &posVBOId, GLuint &texVBOId) {
        ...
        glBufferData(GL_ARRAY_BUFFER, sizeof(float) * DEFAULT_VBO_POS.size(),
                     DEFAULT_VBO_POS.data(), GL_STATIC_DRAW);
        glGenBuffers(1, &texVBOId);
        glBindBuffer(GL_ARRAY_BUFFER, texVBOId);
        glBufferData(GL_ARRAY_BUFFER, sizeof(float) * DEFAULT_VBO_TEX_COORD.size(),
                     DEFAULT_VBO_TEX_COORD.data(), GL_STATIC_DRAW);
        glBindBuffer(GL_ARRAY_BUFFER, 0);
    }
